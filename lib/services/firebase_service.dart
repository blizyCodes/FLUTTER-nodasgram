...
class FirebaseService {
  ...
  Future<bool> registerUser({
    ...
    try {
      //****** creates user in authentications *************
      UserCredential _userCredential = await _auth
          .createUserWithEmailAndPassword(email: email, password: password);
      //get the user's id
      String _userId = _userCredential.user!.uid;
      //create filename for file we need to upload
      String _fileName = Timestamp.now().millisecondsSinceEpoch.toString() +
          p.extension(image.path);
      //we define a task for uploading the file and we define the actual path in firebase storage
      //where we'd like to place our file
      UploadTask _task =
          _storage.ref('images/$_userId/$_fileName').putFile(image);
      return _task.then((_snapshot) async {
        //once complete, we get the download url
        String _downloadURL = await _snapshot.ref.getDownloadURL();
        //create a document under users collections in db
  ...
  Future<bool> loginUser({
    ...
    try {
      UserCredential _userCredential = await _auth.signInWithEmailAndPassword(
          email: email,
          password: password); //*******checks the credentials*******
